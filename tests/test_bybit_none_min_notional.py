#!/usr/bin/env python3
"""
Test to verify that BybitClient handles None values in market limits correctly.
This test validates the fix for the decimal.InvalidOperation error when min_notional returns None.
"""
import io
import os
import sys
...
class _FakeExchange:
    """Mock exchange that returns None for min_notional (simulating Bybit behavior)."""
    ...
    def market(self, symbol):
        """Returns market limits with None for min_notional (cost.min)."""
        return {
            'limits': {
                'amount': {'min': 0.1},
                'cost': {'min': None}  # This is what causes the InvalidOperation error
            },
            'precision': {
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("🧪 Testing BybitClient with None min_notional...")

        client = bybit_client.BybitClient('key', 'secret', testnet=False)

        # Test execute_market_order with a symbol that has None for min_notional
        # This should NOT raise decimal.InvalidOperation anymore
        print("📤 Attempting to execute market order...")
        try:
        ...
        print("✅ BybitClient corretamente trata None em min_notional!")
