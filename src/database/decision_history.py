# -*- coding: utf-8 -*-
"""Histórico de decisões de gestão de posição (Dashboard / SQLite)."""
...
def ensure_historico_decisoes_table() -> None:
    ...
    try:
        ...
        print(f'⚠️ [DECISÕES] ensure table: {err}', flush=True)
...
def record_ia_decision(
    ...
    *,
    ...
    try:
        ...
        if ok:
            print(
                f'🧾 [DECISÕES] {sym} | {tipo_execucao} | {action_payload} | '
                f'PnL~{pnl_garantido_pct:.1f}% — {str(motivo_saida)[:100]}',
                flush=True,
            )
        return ok
    except Exception as err:
        print(f'⚠️ [DECISÕES] record: {err}', flush=True)
        return False
...
def list_ia_decisions(limit: int = 30) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f'⚠️ [DECISÕES] list: {err}', flush=True)
        return []
    finally:
        if conn is not None:
            try:
